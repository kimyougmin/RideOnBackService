...
public class AuthController {
    ...
    public ResponseEntity<Map<String, Object>> register(
        ...
        try {
            ...
            Members savedUser = userRepository.save(member);
    ...
    public ResponseEntity<Map<String, Object>> login(
        ...
        try {
            ...
            Members member = userDetails.getMembers();

            // JWT 토큰 생성
            String accessToken = jwtTokenProvider.createAccessToken(member.getEmail(), member.getId(), member.getEmail());
            String refreshToken = jwtTokenProvider.createRefreshToken(member.getEmail(), member.getId(), member.getEmail());

            // 리프레시 토큰 저장
            tokenManagementService.saveRefreshToken(member.getEmail(), refreshToken);

            log.info("로그인 성공 - 사용자 ID: {}, 이메일: {}", member.getId(), email);
